...
\textbf{Dataset:} Fashion-MNIST with 60,000 training (split 90\% train, 10\% validation) and 10,000 test images. Images normalized to [0, 1] and flattened to 784-dimensional vectors. All experiments were conducted using a fixed random seed for fair comparison across configurations. Each configuration was run once, without averaging over multiple runs; results may vary by approximately 1 to 2\% depending on initialization.
...
We conducted 23 controlled experiments; 6 optimizers, 3 activation functions, 4 architectures, and 10 regularization settings on Fashion-MNIST, systematically varying one hyperparameter while controlling others. All results verified through WandB experiment tracking.
\vspace{-0.7em}
...
We used generative AI for this project only as a supportive tool. It helped us polish grammar, paraphrase parts of the report to stay within the page limit. All code implementation, experimental design, and analysis were completed independently by the students without AI assistance.
